...
void Fit3d_D2::set_no_of_iteration(int _no_of_iteration)
{
	no_of_iteration = _no_of_iteration;
}

void Fit3d_D2::set_values(double *_x, double *_y, double *_errors, double *_x_wires)
{
	// inclined
	x_inclined1[0]       = _x[0];
...
void Fit3d_D2::fit_straight_layer()
{
...
void Fit3d_D2::fit_inclined_layers()
{
	std::vector<double> results;
...
void Fit3d_D2::calculate_wires_xy_functions()
{
	// dla plaszczyzny z = 0
	// straight
	y_x_a_wire[0] = 0;
...
void Fit3d_D2::set_hit_planes_vectors()
{
	// xy plane
...
void Fit3d_D2::calculate_normal_to_hit_planes()
{
...
void Fit3d_D2::calculate_hit_planes_eq()
{
...
void Fit3d_D2::calculate_intersection_vectors() // intersection line: vectors
{
...
void Fit3d_D2::calculate_intersection_points()// intersection line: points
{
...
void Fit3d_D2::calculate_3d_track_parameters()
{
...
void Fit3d_D2::set_detector_position(double _x_lab_position, double _z_lab_position, double _half_x_dim, double _half_z_dim, double _distance_to_1st_layer)
{
...
void Fit3d_D2::calculate_projections_on_hit_planes_calculations() // unused, expect from drawings
{
...
void Fit3d_D2::calculate_projections_on_hit_planes_fit()
{
	double angle;
...
void Fit3d_D2::make_fit_to_lines(bool _unbiased_fit)
{
	...
	lineFit3d_D2 -> LineFit_D2::set_excluded_layer(-1);
	lineFit3d_D2 -> LineFit_D2::fit_with_minuit();
	track3d_fit_point = lineFit3d_D2 -> LineFit_D2::return_track_point();
	// /*std::cout << "in fit " << track3d_fit_point.Y() << std::endl;*/
	track3d_fit_vector = lineFit3d_D2 -> LineFit_D2::return_track_vector(); // normalized to 1 (NOT to uz = 1)
	errflag = lineFit3d_D2 -> LineFit_D2::err_flag();
	chisq = lineFit3d_D2 -> LineFit_D2::get_chisq();
	...
	for (int i = 0; i < 6; i++)
	{
		...
		lineFit3d_D2_Unbiased[i] -> LineFit_D2::set_track_point(track3d_point.X(), track3d_point.Y(), track3d_point.Z());
		lineFit3d_D2_Unbiased[i] -> LineFit_D2::set_track_vector(track3d_vector.X(), track3d_vector.Y(), track3d_vector.Z());
		lineFit3d_D2_Unbiased[i] -> LineFit_D2::calculate_start_params();
		lineFit3d_D2_Unbiased[i] -> LineFit_D2::fit_with_minuit();
		track3d_fit_point_unbiased[i] = lineFit3d_D2_Unbiased[i] -> LineFit_D2::return_track_point();
		track3d_fit_vector_unbiased[i] = lineFit3d_D2_Unbiased[i] -> LineFit_D2::return_track_vector();
		errflag_unbiased[i] = lineFit3d_D2_Unbiased[i] -> LineFit_D2::err_flag();
		chisq_unbiased[i] = lineFit3d_D2_Unbiased[i] -> LineFit_D2::get_chisq();
		lineFit3d_D2_Unbiased[i] -> LineFit_D2::~LineFit_D2();
	}
...
double Fit3d_D2::get_chisq()
{
	return chisq;
}

double Fit3d_D2::get_chisq(int _layer)
{
	return chisq_unbiased[_layer];
}

double Fit3d_D2::get_track_8lines_projection_params(int direction, int a_b)
{
...
TVector3 Fit3d_D2::return_track_point()
{
	return track3d_fit_point;
}

TVector3 Fit3d_D2::return_track_point(int _layer_no)
{
	return track3d_fit_point_unbiased[ _layer_no ];
}

TVector3 Fit3d_D2::return_track_vector()
{
	return track3d_fit_vector;
}

TVector3 Fit3d_D2::return_track_vector(int _layer_no)
{
...
double Fit3d_D2::get_wire_track_dist(int _layer_no)
{
	return wire_track_dist[_layer_no];
}

double Fit3d_D2::calculate_phi_xz()
{
	double phi_xz;
...
double Fit3d_D2::calculate_phi_xz(int _layer_no)
{
	double phi_xz;
	// calculate vector projected onto xz plane
	double vx, vz, norm;
...
double Fit3d_D2::calculate_theta_y()
{
	double theta_y = 0;
	double vz, vy, norm;
	vz = track3d_fit_vector.Z();
...
double Fit3d_D2::calculate_theta_y(int _layer_no)
{
	double theta_y = 0;
	double vz, vy, norm;
	vz = track3d_fit_vector_unbiased[_layer_no].Z();
