...
CalibrationLayer3d_D2::CalibrationLayer3d_D2()
{

}

CalibrationLayer3d_D2::CalibrationLayer3d_D2(int _layer_no, const std::vector<double> &_CalibTimes, const std::vector<double> &_CalibDistances)
{
	max_time_range = -1;
	no_of_corr_bins = -1;
	layer_no = _layer_no;
...
void CalibrationLayer3d_D2::set_no_of_corr_bins(double _no_of_corr_bins)
{
...
void CalibrationLayer3d_D2::set_max_time_range(double _max_time_range)
{
...
void CalibrationLayer3d_D2::set_no_of_bin_in_event()
{
	double dtime;
...
void CalibrationLayer3d_D2::set_no_of_bin_in_calib()
{
	double dtime;
...
void CalibrationLayer3d_D2::set_no_of_iteration(double _no_of_iteration)
{
	no_of_iteration = _no_of_iteration;
}

void CalibrationLayer3d_D2::get_data(double _wire_pos_X, double _wire_pos_Z, double _drift_time, int _lr)
{
	D2_single_event_data3d data;
...
void CalibrationLayer3d_D2::calculate_hit_position()
{
	...
	for (unsigned int i = 0; i < CalibrationData.size(); i++)
	{
		...
		// pos in the lab
		if (layer_no==5||layer_no==6) CalibrationData.at(i).hit_pos_X = wirex+lr*drift_time_to_distance(CalibrationData.at(i).calib_bin, drifttime);//*pow(TMath::Cos(31*TMath::DegToRad()),-1);
		if (layer_no==1||layer_no==2||layer_no==3||layer_no==4) CalibrationData.at(i).hit_pos_X = wirex+lr*drift_time_to_distance(CalibrationData.at(i).calib_bin, drifttime)*pow(TMath::Cos(31*TMath::DegToRad()),-1);
...
double CalibrationLayer3d_D2::drift_time_to_distance(int i, double drift_time) // i - calib bin
{
	double distance;
	int calib_bin = i;
...
double CalibrationLayer3d_D2::set_pos_Xerr()
{
	int corr_bin;
	double error;
	for (unsigned int i = 0; i < CalibrationData.size(); i++)
...
void CalibrationLayer3d_D2::fit_delta_projections(const char* folder_name)
{
...
void CalibrationLayer3d_D2::apply_corrections()
{
	int corr_bin;
...
void CalibrationLayer3d_D2::deletations()
{
	ProjectionConstant.clear();
	ProjectionSigma.clear();
	ProjectionMean.clear();
...
TCanvas* CalibrationLayer3d_D2::plot_delta()
{
	TString name;
...
TCanvas* CalibrationLayer3d_D2::plot_delta_cut()
{
	TString name;
...
TCanvas* CalibrationLayer3d_D2::plot_current_calibration()
{
...
TCanvas* CalibrationLayer3d_D2::plot_corrections()
{
	TString name;
...
void CalibrationLayer3d_D2::calculate_deltas(int i)
{
	double x_wire;
...
/*void CalibrationLayer3d_D2::save_in_root_file(TH2F *_c)
{
...
void CalibrationLayer3d_D2::save_in_root_file(TGraph *_c)
{
	TFile f3("results/calibration_results.root","UPDATE");
	TGraph *c = new TGraph();
	c = _c;
...
void CalibrationLayer3d_D2::save_in_root_file(TH1F *_c)
{
	TFile f3("results/calibration_results.root","UPDATE");
	TH1F *c = new TH1F();
	c = _c;
