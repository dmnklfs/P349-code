#include "LineFit_HEX.h"

LineFit_HEX * LineFit_HEX::_this = NULL;

LineFit_HEX * LineFit_HEX::GetInstance(){ 
  if( _this == NULL ){
    _this = new LineFit_HEX();
  }
  return _this; 
}
...
void LineFit_HEX::set_z_values(double *_z)
{
	for (int i = 0; i < 6; i++)
	{
		z[i] = _z[i];
...
void LineFit_HEX::set_x_errors(double *_errors)
{
	for (int i = 0; i < 6; i++)
	{
		errors[i] = _errors[i];
...
void LineFit_HEX::set_incl_hit_lines_params(double *_a, double *_b)
{
	for (int i = 0; i < 4; i++)
	{
		a[i] = _a[i];
...
void LineFit_HEX::set_track_point(double _track_x, double _track_y, double _track_z)
{
	track_x = _track_x;
	track_y = _track_y;
	track_z = _track_z;
...
void LineFit_HEX::set_track_vector(double _track_ux, double _track_uy, double _track_uz)
{
	track_ux = _track_ux;
	track_uy = _track_uy;
	track_uz = _track_uz;
...
void LineFit_HEX::set_excluded_layer(int _excluded_layer)
{
	excluded_layer = _excluded_layer;
}

bool LineFit_HEX::err_flag()
{
	return errflag;
}
...
void HEXfcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
	LineFit_HEX * rec = LineFit_HEX::GetInstance();
	f = rec->GlobalFCN( par ); 
}

void LineFit_HEX::calculate_start_params()
{
	double t;
	t = 1/track_uz;
	start_ux = t*track_ux;
...
TVector3 LineFit_HEX::return_track_point()
{
...
TVector3 LineFit_HEX::return_track_vector()
{
