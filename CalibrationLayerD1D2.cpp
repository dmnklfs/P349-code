...
CalibrationLayerD1D2::CalibrationLayerD1D2()
{

}

CalibrationLayerD1D2::CalibrationLayerD1D2(int _layer_no, const std::vector<double> &_CalibTimes, const std::vector<double> &_CalibDistances)
{
	max_time_range = -1;
	no_of_corr_bins = -1;
	layer_no = _layer_no;
...
void CalibrationLayerD1D2::set_no_of_corr_bins(double _no_of_corr_bins)
{
...
void CalibrationLayerD1D2::set_max_time_range(double _max_time_range)
{
...
void CalibrationLayerD1D2::set_no_of_bin_in_event()
{
	double dtime;
...
void CalibrationLayerD1D2::set_no_of_bin_in_calib()
{
	double dtime;
...
void CalibrationLayerD1D2::set_no_of_iteration(double _no_of_iteration)
{
	no_of_iteration = _no_of_iteration;
}

void CalibrationLayerD1D2::get_data(double _wire_pos_X, double _wire_pos_Z, double _drift_time, int _lr)
{
	D1D2_single_event_data3d data;
...
void CalibrationLayerD1D2::calculate_hit_position()
{
	...
	for (unsigned int i = 0; i < CalibrationData.size(); i++)
	{
		...
		//std::cout << CalibrationData.at(i).calib_bin << std::endl;
		//std::cout << " a " << std::endl;
		// straight, than inclined
		if (layer_no==5||layer_no==6||layer_no==7||layer_no==8||layer_no==13||layer_no==14) CalibrationData.at(i).hit_pos_X = wirex+lr*drift_time_to_distance(CalibrationData.at(i).calib_bin, drifttime);
		else CalibrationData.at(i).hit_pos_X = wirex+lr*drift_time_to_distance(CalibrationData.at(i).calib_bin, drifttime)*pow(TMath::Cos(31*TMath::DegToRad()),-1);
		// distance wire hit = odległosc prostopadla do drutu!!
		//std::cout <<  layer_no << " k " << i << std::endl;
...
double CalibrationLayerD1D2::drift_time_to_distance(int i, double drift_time) // i - calib bin
{
	double distance;
	int calib_bin = i;
...
double CalibrationLayerD1D2::set_pos_Xerr()
{
	int corr_bin;
	double error;
	for (unsigned int i = 0; i < CalibrationData.size(); i++)
...
void CalibrationLayerD1D2::fit_delta_projections(const char* folder_name)
{
...
void CalibrationLayerD1D2::apply_corrections()
{
	int corr_bin;
...
void CalibrationLayerD1D2::deletations()
{
	ProjectionConstant.clear();
	ProjectionSigma.clear();
	ProjectionMean.clear();
...
TCanvas* CalibrationLayerD1D2::plot_delta()
{
	TString name;
...
TCanvas* CalibrationLayerD1D2::plot_delta_cut()
{
	TString name;
...
TCanvas* CalibrationLayerD1D2::plot_current_calibration()
{
...
TCanvas* CalibrationLayerD1D2::plot_corrections()
{
	TString name;
...
void CalibrationLayerD1D2::calculate_deltas(int i)
{
	double x_wire;
