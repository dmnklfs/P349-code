...
Calibration3d_D2::Calibration3d_D2()
{ 
	std::cout << "Warning: You are probably trying to run a Calibration metod. Note that it requires strict conditions on the number of events in the straight layers of the D2. In case of the problems: check the config/remove this objects/adjust methods of the Calibration class/check event selection criteria in the SingleEvent class. Note that the postion in D2 should be calculated FOR WIRES (now)." << std::endl;
}

Calibration3d_D2::Calibration3d_D2(const Config &_config)
{
	...
	no_of_iteration = 1; 
	no_of_calib_bins = _config.D2_L1_calibration_times.size()-1;
	no_of_corr_bins = no_of_calib_bins;
	max_time_range =  _config.D2_drift_time_max[0];
	...
	std::cout << "* calibration of the D2 DC will be done (with 3d fit)" << std::endl;
	std::cout << "	WARNING: it requires strict conditions on the number of events in all layers of the D2" << std::endl;
...
void Calibration3d_D2::tell_no_of_events()
{
...
void Calibration3d_D2::get_data(data_for_D2_calibration _single_event_data)
{
...
void Calibration3d_D2::set_no_of_iteration(double _no_of_iteration)
{
...
void Calibration3d_D2::calculate_hit_position()
{
	for (int i = 0; i < 6; i++)
	{
...
void Calibration3d_D2::set_no_of_bin_in_event()
{
	for (int i = 0; i < 6; i++)
	{
...
void Calibration3d_D2::save_histograms()
{
	TString name;
...
void Calibration3d_D2::fit_events()
{
	fit_in_3d();
}

double Calibration3d_D2::calculate_phi_xz()
{
	double phi_xz;
...
double Calibration3d_D2::calculate_theta_y()
{
	double theta_y = 0;
	double vz, vy, norm;
	vz = track3d_fit_vector.Z();
...
void Calibration3d_D2::fit_in_3d()
{
	...
	for (unsigned int i = 0; i < no_of_chosen_events; i++)
	{
		...
		Fit3d_D2 *fit3d_D2 = new Fit3d_D2(i);
		fit3d_D2 -> Fit3d_D2::set_no_of_iteration(no_of_iteration);
		fit3d_D2 -> Fit3d_D2::set_z_reference(0);
		fit3d_D2 -> Fit3d_D2::set_values(hits_positionsX_all,hits_positionsZ_all,errors_all, wires_positionsX_all);
		fit3d_D2 -> Fit3d_D2::fit_straight_layer();
		fit3d_D2 -> Fit3d_D2::fit_inclined_layers();
		fit3d_D2 -> Fit3d_D2::calculate_xy_functions();
		fit3d_D2 -> Fit3d_D2::set_hit_planes_vectors();
		fit3d_D2 -> Fit3d_D2::calculate_normal_to_hit_planes();
		fit3d_D2 -> Fit3d_D2::calculate_hit_planes_eq();
		fit3d_D2 -> Fit3d_D2::calculate_intersection_vectors();
		fit3d_D2 -> Fit3d_D2::calculate_intersection_points();
		fit3d_D2 -> Fit3d_D2::calculate_3d_track_parameters();
		fit3d_D2 -> Fit3d_D2::set_detector_position(x_lab_position, z_lab_position, half_x_dim, half_z_dim, distance_to_1st_layer);
		fit3d_D2 -> Fit3d_D2::calculate_projections_on_hit_planes_calculations();

		fit3d_D2 -> Fit3d_D2::make_fit_to_lines(unbiased_fit);
		fit3d_D2 -> Fit3d_D2::calculate_projections_on_hit_planes_fit();
		track3d_fit_point = fit3d_D2 -> Fit3d_D2::return_track_point();
		...
		if ((!(fit3d_D2 -> Fit3d_D2::err_flag())))//&&((temp_chi2_prob>0.05)||no_of_iteration==0))
		{
			...
			bSt = fit3d_D2 -> Fit3d_D2::get_track_8lines_projection_params(0,1);
			...
			if ( was_correct_angle(track_angle) ) //was_correct_angle(track_angle) )
			{
				...
				phi_xz -> Fill(Calibration3d_D2::calculate_phi_xz());
				theta_y -> Fill(Calibration3d_D2::calculate_theta_y());
...
void Calibration3d_D2::deletations()
{
	for (int i = 0; i < 6; i++)
	{
...
TCanvas* Calibration3d_D2::plot_chi2()
{
	TString name;
	name = "c_#chi^{2}_1";
...
TCanvas* Calibration3d_D2::plot_chi2_cut()
{
	TString name;
	name = "c_#chi^{2}_2";
	TCanvas *c = new TCanvas(name,name);
...
TCanvas* Calibration3d_D2::plot_chi2_cut(int _layer_no)
{
	TString name;
...
TCanvas* Calibration3d_D2::plot_chi2_pdf()
{
	TString name;
	name = "c_#chi^{2}_pdf";
...
TCanvas* Calibration3d_D2::plot_chi2_pdf(int _layer_no)
{
	TString name;
...
void Calibration3d_D2::fit_delta_projections()
{
	TString name;
	for (int i = 0; i < 6; i++)
	{
...
void Calibration3d_D2::apply_corrections()
{
	for (int i = 0; i < 6; i++)
	{
...
void Calibration3d_D2::set_pos_Xerr()
{
	for (int i = 0; i < 6; i++)
	{
...
void Calibration3d_D2::plot_current_calibration()
{
	TString name;
	for (int i = 0; i < 6; i++)
	{
...
TCanvas* Calibration3d_D2::plot_angle_distribution()
{
	TString name;
...
TCanvas* Calibration3d_D2::plot_angle_distribution(int _layer_no)
{
	TString name;
...
bool Calibration3d_D2::was_correct_angle(double track_angle)
{
